...
const LEVELS = [
  ...
  // Stage 2-1：認識煙囟橘子
  {
    ...
    buildPlatforms: () => [],
    buildSpikes:        () => [],
    // v0.3.20-economy：2-1 教學關，約 17 枚金幣（每枚 1 金幣）
    // 主要沿路平均分布，少量集中在煙囟橘子本體附近（地面安全但靠近威脅來源，培養觀察習慣）
    buildCoins: () => {
      const placements = [
        [300,55],[500,55],
        [880,55],[940,55],[1060,55],[1120,55],   // 1000 煙囟橘子周圍
        [1500,55],[1750,55],[2000,55],
        [2280,55],[2340,55],[2460,55],[2520,55], // 2400 煙囟橘子周圍
        [2900,55],[3150,55],[3400,55],
        [4100,55],
      ];
      return placements.map(([x,yOff]) => ({ x, y: GROUND_Y-yOff, collected:false, bobOffset:Math.random()*Math.PI*2 }));
    },
  ...
  // Stage 2-2：抓噴油時機通過
  {
    ...
    buildCoins: () => {
      const placements = [
        // 前段：煙囟橘子時機訓練區，沿路平均分布，少量緊鄰本體
        [250,55],[600,55],
        [780,55],[850,55],[1020,55], // 900 煙囟橘子周圍
        [1300,140],                  // 尖刺旁，需精準跳躍
        [1600,55],[1950,55],
        [2150,55],[2750,55],[2820,55], // 2700 煙囟橘子周圍
        [3200,55],[3500,55],
        [3550,55],[3850,55],           // 3700 煙囟橘子周圍
        [4200,140],                    // 尖刺旁
        [4500,55],[4900,55],
        [4650,55],[4750,55],           // 4700 煙囟橘子周圍
        [5300,55],
        // 後段：雙噴嘴橘子區域 — 高處平台上放金幣（鼓勵走安全路線）
        [6150,150],[6200,150],[6580,150],[6620,150],
        // 後段：地面靠近雙噴嘴橘子噴油範圍邊緣（真正的風險回報，靠近危險才拿得到）
        [6050,55],[6480,55],
        [7350,150],[7400,150],
      ];
      return placements.map(([x,yOff]) => ({ x, y: GROUND_Y-yOff, collected:false, bobOffset:Math.random()*Math.PI*2 }));
  ...
  // Stage 2-3：蠍子 + 煙囟橘子 複合挑戰
  {
    ...
    buildCoins: () => {
      const placements = [
        // 前段：純蠍子複習（700~1800），沿地面平均分布
        [350,55],[600,55],[850,55],[1100,55],[1400,55],[1900,55],
        // 中段：純煙囟橘子複習（2400~3600），少量緊鄰本體
        [2150,55],[2350,55],[2460,55],
        [3550,55],[3650,55],[3750,55],
        // 段3：蠍子+煙囟橘子混合（4200~5800），開始升高密度
        [3950,55],[4150,140],            // 尖刺旁，需精準跳躍
        [4750,55],[4820,55],[4880,55],   // 4800 煙囟橘子周圍
        [5200,55],[5450,55],
        [5950,55],[6020,55],[6080,55],   // 6000 煙囟橘子周圍
...
// v0.3.17：蠍子擊殺回饋時序常數
// 閃爍（紅/白交替）→ 淡出上彈，整體 18 frames ≈ 0.3s，短促有擊殺感
const SCORPION_DEFEAT_TOTAL_FRAMES = 18;
...
function updateScorpionDefeatEffects(dt) {
  for (let i = scorpionDefeatEffects.length - 1; i >= 0; i--) {
    ...
    fx.life--;
    fx.y += (fx.vy || 0) * dt;
    if (fx.timer >= fx.totalFrames) scorpionDefeatEffects.splice(i, 1);
  }
}
...
function drawOrangeNemesis(sx, o) {
  ...
  if (useSkin) {
    if (o.phase === 'windup') {
      ...
      // v0.3.14-test-3：
      // 1. ORANGE_COOLDOWN_FRAME_DUR=30 → 每 30 frames 換一張，避免閃爍
      // 2. info.drawY + ORANGE_COOLDOWN_DRAW_OFFSET_Y → 讓腳底貼近地平線
      // 3. 慢速透明度呼吸（0.82~1.00），呈現疲憊喘氣感，不是受傷閃爍
      const useFirst = (Math.floor(frameCount / ORANGE_COOLDOWN_FRAME_DUR) % 2 === 0);
      ...
      if (cdImg) {
        ...
        if (info) {
          ...
          ctx.drawImage(cdImg,
            info.drawX,
            info.drawY + ORANGE_COOLDOWN_DRAW_OFFSET_Y,
            info.drawW,
            info.drawH);
          ctx.restore();
        }
